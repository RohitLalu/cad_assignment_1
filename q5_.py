...
def full_overlap_q5():
    ...
    for i in range(n):
        ...
        for j in range(n):
            if (i != j) and (blocks_list[j].block_id not in all_overlapping_blocks):
                overlap_type,ignore = partial_overlap_check(blocks_list[j], blocks_list[i]) #check if j in i (not vice versa)
                if overlap_type == 2:
        ...
        if len(overlap_list) > 1:  # only add if there are full overlaps
    ...
    t3=pb.plot_blocks(all_overlapping_blocks, show=False,title="Fully Overlapping Blocks")
    plt.show()
